...
// if nowplaying is appended to sidepanel, use large image, else extra large for mobile view
    if (data.album && data.album.name) {
            if ($(window).width() > 724) {
            $("#modalalbum").html('<a href="#" onclick="return showAlbum(\'' + data.album.uri + '\');">' + data.album.name + '</a>');
            getCover(artiststext, data.album.name, '#infocover, #controlspopupimage', 'large');
            } else {
            $("#modalalbum").html('<a href="#" onclick="return showAlbum(\'' + data.album.uri + '\');">' + data.album.name + '</a>');
	...
	$("#infocover").attr('src', '../images/default_cover.png');
                    if ($(window).width() > 724) {
                    $("#controlspopupimage").attr('src', '../images/default_cover_small.png');
                    } else {
...
//    $('#trackslider').removeClass('ui-btn-corner-all');
//    $('#trackslider').addClass('ui-btn-corner-none');
...
function locationHashChanged() {
    ...
    //switch the footer but show controls always if sidepanel expanded
    switch(divid) {
        case 'nowPlaying':
            $('#normalFooter').hide();
...
//    mopidy.on(console.error.bind(console));
  ...
  // move volume slider to header if small screen for easy access and hide the now empty li's so not ugly
        $("#vol1").prependTo("#vol2");
        $("#mutebt").attr('style', 'float:right');
        $('#vol_li').hide();
        $('#nowplay_li').hide();
        $('#navnowPlaying').show();
  //    $("#playlistspane").attr('style', 'padding-top:20px');
