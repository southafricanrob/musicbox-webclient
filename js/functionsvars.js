...
//console.log(results, target, uri);
	...
	nextname = '';
//this seems to hide the breaking into albums but saves single album playlists from breaking
...
//                if ( newalbum.length > 0 ) {
		    ...
		    '<a href="#" onclick="return ' + playlistType + '(\'' + newalbum[0].uri + '\',\'' + uri + '\');">' +
                    '<h1>' + newalbum[0].name + "</h1>" + "<span style='color:grey'>" + ' by ' + "</span>";
...
// this hides album name completely, uncomment block to show in grey, album shows in popup so not really needed and uses space
//                if ( newalbum[0].album.name != '') { html += "<span style='color:grey'>" +' from ';}
//		html += '<em>' + newalbum[0].album.name + '</em></span>';
//                html += '</a></li>';
